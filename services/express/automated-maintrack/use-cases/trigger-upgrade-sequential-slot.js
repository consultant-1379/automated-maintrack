function makeTriggerUpgradeSequentialSlot(
  failureTrackerService, slotService, jenkinsService, checkForTestEnvironmentAvailableDeliveryGroups,
  checkForAvailableTestEnvironments, checkForOngoingSlots, checkForDeliverDeliveryGroups,
  useExternalApiService, determineSlotNumber, request,
) {
  return async function triggerUpgradeSequentialSlot() {
    const upgradeDeployPhase = 'upgrade';
    const maximumNumberOfRunningSlots = 1;
    ...
    const newSlotNumber = await determineSlotNumber(fullSlotListBody, emailRecipient);
...
module.exports = {
  makeTriggerUpgradeSequentialSlot,
};
